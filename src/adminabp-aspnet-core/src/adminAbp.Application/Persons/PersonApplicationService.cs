...
using adminAbp.Persons.DomainService;
...
namespace adminAbp.Persons
{
    ...
    ///</summary>
    [AbpAuthorize]
    public class PersonAppService : adminAbpAppServiceBase, IPersonAppService, ITransientDependency
    {
        //ef配置
        private readonly IRepository<Person, int> _entityRepository;
        private readonly IPersonManager _entityManager;
        //ITransientDependency 继承，配置Abp.Dapper 
        ...
        ,IPersonManager entityManager, IRepository<Persons.Person> personRepository,
...
PersonEditDto editDto;
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<PersonListDto>> GetPersonListAsync()
        {
            var result = await _entityRepository.GetAllListAsync();
           return result.MapTo<List<PersonListDto>>();
        }
