...
PersonMapperMapper.CreateMappings(configuration);
```
...
```
Configuration.Authorization.Providers.Add<PersonMapperAuthorizationProvider>();
...
可以在```DbContext```中增加：
 ...
 ```
public DbSet<PersonMapper>  PersonMappers { get; set; }
...
```
        ...
        {
            modelBuilder.ApplyConfiguration(new PersonMapperCfg());
        }
